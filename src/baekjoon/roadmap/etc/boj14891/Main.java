...
public class Main {
	...
	static boolean[] same = new boolean[5];
	...
	public static void turnWheel(int target, int clock, int direction) {
		...
		// 좌향일 경우 오른쪽 극 동일 여부 최신화
		if(direction == -1) {
			same[target] = (arr[target][right[target]] == arr[target + 1][left[target + ()]]);
		}
		// 우향일 경우 왼쪽 극 동일 여부 최신화
		if(direction == 1) {
			same[target] = (arr[target][left[target]] == arr[target - 1][right[target]]);
		}

		// 톱니 회전 전파
		if(direction == 0) {
			// 시작 톱니(0)일 경우 양방향 전파
			if(leftSame) turnWheel(target - 1, -direction, -1);
			if(rightSame) turnWheel(target + 1, -direction, 1);
		} else if(direction == -1) {
			// 좌향일 경우 왼쪽으로만 전파
			if(leftSame) turnWheel(target - 1, -direction, -1);
		} else if(direction == 1) {
